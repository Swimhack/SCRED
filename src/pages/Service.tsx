      ...
      <div className="bg-image-content pt-24 pb-16">
        <div className="container mx-auto px-4">
          <div className="bg-gray-900/80 backdrop-blur-sm rounded-xl p-8 md:p-12 shadow-xl text-white">
            <h1 
              className="text-4xl md:text-5xl font-bold mb-8 text-brand-bittersweet"
              style={{
            ...
            <div className="max-w-2xl mx-auto">
              {services.map((service, index) => (
                <div key={index} className="bg-gray-800/70 rounded-lg p-6 shadow-lg hover:shadow-xl transition-all">
                  <h2 className="text-2xl font-bold mb-4 text-brand-bittersweet">{service.title}</h2>
                  <p className="mb-6 text-gray-300">{service.description}</p>
                  <ul className="space-y-3">
                    {service.features.map((feature, idx) => (
                      <li key={idx} className="flex">
                        <Check className="text-brand-bittersweet mr-2 h-5 w-5 mt-0.5 flex-shrink-0" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
            ...
            <div className="mt-16 text-center">
              ...
              <a href="/contact" className="bg-brand-maize text-black px-8 py-3 rounded-full font-medium hover:bg-brand-maize/90 transition-colors inline-block">
                Contact Us
              </a>
            </div>
          </div>
