...
AS $$
  SELECT COALESCE(
    (SELECT * FROM public.notification_preferences WHERE user_id = user_id_param),
    ROW(
      gen_random_uuid(), -- id
      user_id_param, -- user_id
      true, -- email_notifications
      'immediate', -- email_frequency
      true, -- developer_messages_email
      true, -- system_alerts_email
      true, -- application_updates_email
      false, -- sms_enabled
      NULL, -- phone_number
      false, -- phone_verified
      true, -- sms_critical_only
      true, -- browser_notifications
      NULL, -- push_subscription
      true, -- sound_enabled
      true, -- haptic_enabled
      ...
      'UTC', -- timezone
      ARRAY['email', 'sms', 'push'], -- critical_channels
      ARRAY['email', 'push'], -- high_channels
      ARRAY['email'], -- normal_channels
      ARRAY['dashboard'], -- low_channels
      false, -- escalation_enabled
      30, -- escalation_delay_minutes
      NULL, -- escalation_recipient_id
      now(), -- created_at
      now() -- updated_at
